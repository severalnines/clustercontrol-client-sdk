  ...
  class StatCmonAgent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::StatCmonAgent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::StatCmonAgent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      operation_validator = EnumAttributeValidator.new('String', ["/cmonagent"])
      return false unless operation_validator.valid?(@operation)
      class_name_validator = EnumAttributeValidator.new('String', ["CmnRequest"])
    ...
    def operation=(operation)
      validator = EnumAttributeValidator.new('String', ["/cmonagent"])
    ...
    def class_name=(class_name)
      validator = EnumAttributeValidator.new('String', ["CmnRequest"])
    ...
    def hash
      [operation, api_key, class_name, request_data].hash
    end
